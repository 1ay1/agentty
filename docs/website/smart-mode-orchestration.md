...
:::tip
Watching the 🧠 card + the `task` cards together is the whole story of a turn: *what* agentty decided, then *how* it executed. If a turn feels over- or under-powered, the card tells you why — and each turn's header names the model that actually served it, so a delegation trace is readable straight from the transcript.
:::
