...
| `--provider <p>` | LLM backend: `anthropic` (default) or an OpenAI-compatible one — `openai` · `chatgpt` · `copilot` · `kimi` · `deepseek` · `gemini` · `xai` · `mistral` · `groq` · `openrouter` · `yolo-auto` · `together` · `cerebras` · `fireworks` · `ollama`, a raw `host:port`, or a full URL `https://host/path` for servers with a custom path prefix. Persisted like `-m`; switch live with `^P`. See [Providers & Models](/docs/providers). |
