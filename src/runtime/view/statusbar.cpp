...
Element status_bar(const Model& m) {
    ...
    bool is_err = has_status
                  && m.s.status.rfind("error:", 0) == 0;
    ...
    return maya::StatusBar{{
        ...
        .breadcrumb_title = m.d.current.title,
        .token_stream = {
            .show    = true,
            .rate    = disp_rate,
        ...
        .status_text     = has_status ? m.s.status : std::string{},
        .status_is_error = is_err,
        .shortcuts       = std::move(shortcuts),
        // Streaming pushes the breadcrumb threshold up so the live
        // sparkline + tok/s readout has room to breathe without
        // elbowing the title.
        .breadcrumb_min_width   = is_streaming ? 160 : 130,
        .token_stream_min_width = 110,
        .ctx_bar_min_width      = 55,
        .text_color             = fg,
    }}.build();
}
