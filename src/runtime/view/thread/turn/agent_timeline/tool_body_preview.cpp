    ...
    const ToolUse& tc, const GrepHits* grep_hits)
{
    ...
    // ── Edit
    //
    //  Three rendering paths, picked by what data we have:
    //
    //  (a) STRUCTURED — if `tc.file_change` is present (post-tool, in
    //      either fresh-this-session or loaded-from-thread state), we
    //      have real Hunk records with `old_start`/`new_start` line
    //      numbers and the unified-diff `patch` per hunk. Synthesize
    //      the unified-diff bytes from the structured form and route
    //      through Kind::GitDiff for interleaved −/+ coloring + line
    //      anchors. Identical in shape to a real `git diff` body —
    //      same widget, same glyphs — but driven from the typed
    //      hunks the dispatcher attached, not from text round-trip.
    //      Also drives the diff-review pane (^D) under the hood.
    //
    //  (b) FENCE-PARSE — fallback for terminal-state Edits where the
    //      structured change is missing (older threads saved before
    //      file_change-on-ToolUse, custom tool wrappers): pull the
    //      diff payload out of the ```diff … ``` block the edit tool
    //      writes into its output text. Still routes through
    //      Kind::GitDiff, just less robust.
    //
    //  (c) ARGS-ECHO — streaming / pre-run, no diff exists yet:
    ...
    //      lands.
    if (n == "edit") {
        if (tc.file_change && !tc.file_change->hunks.empty()) {
            const auto& fc = *tc.file_change;
            std::string unified;
            unified.reserve(64 * fc.hunks.size());
            for (const auto& h : fc.hunks) {
                unified += "@@ -" + std::to_string(h.old_start) + ","
                         + std::to_string(h.old_len) + " +"
                         + std::to_string(h.new_start) + ","
                         + std::to_string(h.new_len) + " @@\n";
                unified += h.patch;
                if (!h.patch.empty() && h.patch.back() != '\n')
                    unified += '\n';
            }
            out.kind = Kind::GitDiff;
            out.text = std::move(unified);
