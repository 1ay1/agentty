    ...
    return std::visit(overload{
            ...
            for (auto& c : m.d.provider_catalogs) {
                ...
                if (e.ok && !e.models.empty()) {
                    ...
                    for (auto& mi : c.models) {
                        ...
                        // never registered under dev_scope. Claim by the
                        // catalog's own origin (model-id qualifier + port
                        // rule inside), so a localhost row whose models.dev
                        // entry matches its URL shows the declared window.
                        if (mi.context_window == 0) {
                            std::uint16_t port = 0;
        ...
        [&](ModelsLoaded& e) -> Step {
            ...
            for (auto& mi : e.models) {
                ...
                if (mi.context_window == 0) {
                    const std::string pid = e.provider_id.empty()
                                                ? active_provider_id()
                                                : e.provider_id;
                    std::uint16_t port = 0;
                    const std::string host =
