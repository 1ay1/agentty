...
std::string default_system_prompt(bool lean) {
    ...
    oss << "You are agentty, a terminal coding assistant. Act, don't ask. "
        ...
        << "Metavariables `$X` (one node/group) and `$$$X` (many) match "
        ...
        << "  For structural navigation of a KNOWN symbol, `find_definition` "
        << "/ `find_references` beat grep (they return the enclosing symbol "
        << "and skip false hits). And always let the expensive layer NARROW "
        << "scope, then finish with the cheap one \xe2\x80\x94 e.g. `repo_map` to "
        << "find the 3 files a subsystem lives in, then `grep` within them. "
        << "Never dump whole files when a `file:line` from any of these "
        << "would do.\n"
        << "</search-strategy>\n\n"
        << "<in-house-languages>\n"
