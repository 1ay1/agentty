...
// The "limit.context" figure from the models.dev snapshot. Same precedence
// posture as the effort ladder: a user override or a LIVE advertisement
// (probed /v1/models, Ollama /api/show) outranks it — see
// ui::resolve_context_window. Only consulted for models the endpoint said
// NOTHING about, so it fixes the "every custom-host model reads 200k" case
// without touching gateways that do report.
...
// Bare-tail merge — same cross-provider collision semantics as
// merge_catalog_reasoning: agree → keep, disagree → poison to "no
// declaration" so resolution falls back to the scoped fact / id-inference.
inline void merge_catalog_context_window(const std::string& raw_id,
                                         int tokens) {
    if (tokens <= 0) return;
    const std::string model_id = norm_caps_key(raw_id);
    std::unique_lock lk(catalog_context_detail::mu());
    ...
    if (poisoned.count(model_id)) return;
    if (auto it = m.find(model_id); it != m.end()) {
        if (it->second != tokens) { m.erase(it); poisoned.insert(model_id); }
        return;
    }
    m[model_id] = tokens;
    catalog_context_detail::any().store(true, std::memory_order_relaxed);
...
// 0 = no declaration; else the declared context window in tokens.
// Scoped-first ("provider/model"), bare fallback — see scoped_caps_key.
[[nodiscard]] inline int catalog_context_window_for(
    std::string_view model_id, std::string_view scope = {}) {
    if (!catalog_context_detail::any().load(std::memory_order_relaxed))
        return 0;
    std::shared_lock lk(catalog_context_detail::mu());
    auto& m = catalog_context_detail::map_();
