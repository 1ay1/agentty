...
inline constexpr std::array kCommands = std::array{
    ...
    CommandDef{Command::SmartMode,     "Smart Mode",         "Route cheap grunt work (compaction, retrieval) to a cheaper model — toggle on/off", ""},
    CommandDef{Command::SmartSetStrategic,"Smart: set Strategic model","Pick the model for planning / hard reasoning", ""},
    CommandDef{Command::SmartSetImpl,  "Smart: set Implementation model","Pick the model for writing code / edits", ""},
    CommandDef{Command::SmartSetUtility,"Smart: set Utility model","Pick the cheap model for grunt work (compaction, retrieval)", ""},
