...
\begin{aside}
A promise, up front: this book is meant to be \emph{fun}. Terminals
are strange, beautiful, and full of stories --- about a 1978 hardware
terminal that refused to die, about programmers who taught text to
think it was a pixel, about ANSI committees who never imagined their
bytes would still be in use forty-seven years later. We are going to
tell those stories \emph{while} we teach you to build with \maya{}.
If at any point you find yourself bored, you have my permission to
flip ahead, run the code, break something, and circle back. Books on
libraries should be played with, not just read.
\end{aside}
...
\begin{funfact}
The VT100 shipped the year \emph{Grease} was in cinemas and the Sony
Walkman did not yet exist. The escape sequences you will learn in
Chapter~\ref{ch:ansi} --- the ones that move your cursor and paint
colour --- predate the mouse, the World Wide Web, USB, and most of
the people reading this book. When your \code{htop} repaints, it is
literally speaking 1978 at your screen sixty times a second, and your
screen is going along with it.
\end{funfact}
...
\begin{tryit}
Open your terminal and run \code{htop} (install it if you do not have
it: \code{sudo apt install htop} on Debian/Ubuntu, \code{brew install
htop} on macOS). Resize the window. Watch the columns rearrange in
perfect grid steps --- never half a character wide. That is the TUI
rule made visible: characters in, characters out, nothing in between.
Press \code{q} to quit. You have just used the same kind of program
you are about to learn to build.
\end{tryit}
...
Think of \maya{} as the difference between writing a letter and
speaking the words yourself. Without a library you are shouting bytes
into the void: \code{ESC[2J}, \code{ESC[H}, \code{ESC[31m}, ``red
please, row 1 column 1, clear the screen first''. With \maya{} you
say \emph{``a red title at the top''} and the library does the
shouting --- in 1978's dialect, no less --- on your behalf.
...
\begin{mythbust}
\textbf{Myth:} ``\code{ncurses} is the terminal.''\\
\textbf{Reality:} \code{ncurses} is a 1980s C library that wraps a
1970s terminal-capability database (\code{terminfo}) so programs do
not have to special-case every model of physical terminal. It is
venerable, it is everywhere, and it is showing its age. Modern
libraries like \maya{} talk ANSI directly to the emulator and skip
the \code{terminfo} layer entirely --- because every emulator made
in the last twenty years already agreed to speak ANSI. The terminal
is not \code{ncurses}; the terminal is bytes, and \code{ncurses} is
one way to spell them.
\end{mythbust}
